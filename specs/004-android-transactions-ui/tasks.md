...
- [X] T001 Agregar `junit` y `kotlinx-coroutines-test` (misma versión que `kotlinx-coroutines-android`)
      a `android/gradle/libs.versions.toml` y como dependencias `testImplementation` en
      `android/app/build.gradle.kts`
- [X] T002 [P] Crear el paquete `android/app/src/main/java/com/walletapp/android/transactions/` (y su
...
- [X] T003 Crear `transactions/TransactionApi.kt`: DTOs `TransactionRequest`, `TransactionUpdateRequest`,
...
- [X] T004 Crear `transactions/TransactionRepository.kt`: `list(filter)`, `create(...)`, `update(id, ...)`,
...
- [X] T005 [P] Registrar `provideTransactionApi()` en `di/NetworkModule.kt` — depende de T003
...
- [X] T006 [US1] Implementar `TransactionsUiState` (Loading/Success/Error) y `TransactionViewModel`
...
- [X] T007 [P] [US1] Tests unitarios de `TransactionViewModel` (lista carga bien / error de red, alta
      ...
      depende de T006
- [X] T008 [US1] Implementar `transactions/ui/TransactionListScreen.kt`: lista de movimientos (tipo,
      monto, fecha, cuenta), distinguiendo visualmente ingreso/gasto (FR-003), estado vacío, FAB para
      nuevo movimiento — depende de T006
- [X] T009 [US1] Implementar `transactions/ui/TransactionFormScreen.kt` (alta): campos tipo, monto,
...
- [X] T010 [US1] Agregar `Screen.TransactionsList`/`Screen.TransactionForm`, su entrada en el mapa de
      `BackHandler`, y el botón "Mis movimientos" en `HomeScreen` de `MainActivity.kt` — depende de
      T008, T009
- [X] T011 [US1] Guardia de "sin cuentas propias": si el usuario no tiene ninguna cuenta, el punto de
...
- [X] T012 [US2] Agregar `AccountWithBalance` y modificar `AccountsUiState.Success` para exponer
...
- [X] T013 [P] [US2] Tests unitarios de `AccountViewModel` (saldo se resuelve por cuenta; error al
      ...
      T012
- [X] T014 [US2] Actualizar `AccountRow` en `accounts/ui/AccountListScreen.kt` para mostrar
...
- [X] T015 [US3] Agregar `update(id, ...)` y `delete(id)` con callback `Result<Unit>` a
      `TransactionViewModel.kt` (mismo patrón que evita el bug de fallo silencioso de la feature 002) —
      depende de T006
- [X] T016 [P] [US3] Tests unitarios de edición/eliminación (éxito y error) en
      `TransactionViewModelTest.kt` — depende de T015
- [X] T017 [US3] Extender `TransactionFormScreen.kt` a modo edición: parámetro
      ...
      T009, T015
- [X] T018 [US3] Conectar el tap sobre una fila de `TransactionListScreen.kt` a
...
- [X] T019 [US4] Agregar `TransactionFilterState` (accountId/categoryId/dateFrom/dateTo) al
...
- [X] T020 [P] [US4] Tests unitarios de las transiciones de estado al aplicar/quitar filtros en
      `TransactionViewModelTest.kt` — depende de T019
- [X] T021 [US4] Agregar fila de `FilterChip` (cuenta/categoría/rango de fechas) con
...
- [X] T022 Ejecutar en un dispositivo físico los 7 escenarios de `quickstart.md`, incluyendo los edge
      cases de error de red (#6) y usuario sin cuentas (#7)
- [X] T023 [P] Revisar consistencia de mensajes de error entre `TransactionFormScreen`,
      `TransactionListScreen` y `AccountListScreen` (mismo tono/formato ya usado en cuentas/categorías)
- [X] T024 Confirmar `./gradlew :app:testDebugUnitTest` en verde con todos los tests de esta feature
...
## Bugs reales encontrados y corregidos durante T022 (validación manual en dispositivo físico)

- **Saldo de cuenta desactualizado tras registrar un movimiento**: `AccountViewModel` se retiene entre
  navegaciones (no hay un `NavHost` real con destrucción de back-stack, ver `MainActivity.kt`), y
  `TransactionListScreen`/`TransactionFormScreen` ya lo inyectan para poblar el selector de cuentas —
  por lo tanto, para cuando el usuario visita por primera vez "Mis cuentas", esa instancia de
  `AccountViewModel` ya existía y su `init { refresh() }` ya se había ejecutado una única vez, antes de
  que existiera el movimiento recién creado. Resultado observado: el saldo mostrado quedaba congelado en
  su valor inicial (o el de la última vez que se entró a esa pantalla) en vez de reflejar el movimiento
  recién guardado. Corregido agregando `LaunchedEffect(Unit) { viewModel.refresh() }` en
  `AccountListScreen.kt`, para que el saldo se vuelva a pedir cada vez que se entra a esa pantalla.
- **Botones inalcanzables en el formulario de movimiento**: el `Column` raíz de
  `TransactionFormScreen.kt` no tenía `verticalScroll`; en el flujo de edición, con la fila opcional de
  categoría visible, el contenido superaba el alto de pantalla del dispositivo de prueba y dejaba
  "Eliminar movimiento" y "Cancelar" fuera del área visible y sin forma de hacer scroll para
  alcanzarlos. Corregido agregando `.verticalScroll(rememberScrollState())` al modifier del `Column`.
  El mismo patrón (`Column` sin scroll) existe en `AccountFormScreen.kt`/`CategoryFormScreen.kt`
  (feature 002) pero corregirlo ahí queda fuera de alcance de esta spec — se dejó una tarea de
  seguimiento aparte.
