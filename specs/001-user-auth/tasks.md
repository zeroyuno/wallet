...
- [x] T001 Crear migración Flyway `backend/src/main/resources/db/migration/V1__create_users_and_revoked_tokens.sql`
      con las tablas `users` y `revoked_tokens` (ver [data-model.md](data-model.md))
- [x] T002 [P] Añadir dependencias `io.jsonwebtoken:jjwt-api`, `jjwt-impl`, `jjwt-jackson` en
      `backend/pom.xml`
- [x] T003 [P] Añadir dependencia `androidx.security:security-crypto` en
...
- [x] T004 [P] Crear value objects `UserId`, `Email`, `PasswordHash` en
      `backend/src/main/java/com/walletapp/backend/auth/domain/`
- [x] T005 [P] Crear agregado `User` (factory `register`, `verifyPassword`,
      `registerFailedLogin`/`registerSuccessfulLogin`) en `auth/domain/User.java`
- [x] T006 [P] Crear entidad `RevokedToken` en `auth/domain/RevokedToken.java`
- [x] T007 [P] Crear puertos de salida `UserRepository`, `RevokedTokenRepository`, `PasswordHasher`
      (interfaces) en `auth/domain/`
- [x] T008 [P] Crear excepciones de dominio (`EmailAlreadyInUseException`,
      `InvalidCredentialsException`, `AccountLockedException`) en `auth/domain/exception/`
- [x] T009 [P] [ARQUITECTURA] Test ArchUnit: `auth.domain` no importa `org.springframework.*` ni
...
- [x] T010 Crear puerto `TokenIssuer` (interfaz) en `auth/application/TokenIssuer.java`
- [x] T011 [P] Crear DTOs de aplicación (`RegisterCommand`, `LoginCommand`, `AuthResult`, `UserView`)
      en `auth/application/dto/`

### Infrastructure — persistencia y seguridad transversal

- [x] T012 [P] Crear `UserEntity`/`RevokedTokenEntity` (JPA) y `JpaUserRepository`/
      `JpaRevokedTokenRepository` (implementan los puertos de T007) en
      `auth/infrastructure/persistence/` (depende de T004-T007)
- [x] T013 [P] Implementar `BCryptPasswordHasher` (implementa `PasswordHasher`) en
      `auth/infrastructure/security/BCryptPasswordHasher.java`
- [x] T014 Implementar `JjwtTokenIssuer` (implementa `TokenIssuer`, genera JWT con `jti`) en
      `auth/infrastructure/security/JjwtTokenIssuer.java` (depende de T010)
- [x] T015 Implementar `JwtTokenValidator` (valida firma/`exp`, consulta `RevokedTokenRepository`) en
...
- [x] T016 Implementar `JwtAuthFilter` + `SecurityConfig` (registra el filtro, reglas públicas vs.
      protegidas) en `backend/src/main/java/com/walletapp/backend/shared/security/` (depende de T015)
- [x] T017 [P] [ARQUITECTURA] Test ArchUnit: ningún paquete de un contexto (`auth`, futuros) importa
...
- [x] T018 [P] [US1] Test unitario de `User.register`/`RegisterUserUseCase` (sin Spring ni DB — mocks
      de los puertos) en `backend/src/test/java/com/walletapp/backend/auth/application/RegisterUserUseCaseTest.java`
- [x] T019 [P] [US1] Test de integración de `POST /api/auth/register` (éxito y email duplicado) con
      Testcontainers en `backend/src/test/java/com/walletapp/backend/auth/infrastructure/AuthControllerIT.java`

### Implementation for User Story 1

- [x] T020 [US1] Implementar `RegisterUserUseCase` en `auth/application/RegisterUserUseCase.java`
      (depende de T005, T007, T008, T011)
- [x] T021 [US1] Implementar `POST /api/auth/register` en `auth/infrastructure/web/AuthController.java`
      (mapea DTOs web ↔ DTOs de aplicación; depende de T020)
- [x] T022 [P] [US1] Crear `RegisterScreen.kt` (Compose) en
      `android/app/src/main/java/com/walletapp/android/auth/ui/RegisterScreen.kt`
- [x] T023 [US1] Implementar `AuthApi.register` (Retrofit) y `AuthRepository.register` en Android
- [x] T024 [US1] Implementar `AuthViewModel` con el estado de registro (depende de T023)
...
- [x] T025 [P] [US2] Test unitario de `LoginUseCase` incluyendo el conteo de intentos fallidos y
      bloqueo temporal (FR-010) en `auth/application/LoginUseCaseTest.java`
- [x] T026 [P] [US2] Test de integración de `POST /api/auth/login` (éxito, credenciales inválidas,
      bloqueo por rate limiting) en `AuthControllerIT.java`

### Implementation for User Story 2

- [x] T027 [US2] Implementar `LoginUseCase` (valida credenciales vía `user.verifyPassword`, aplica
...
- [x] T028 [US2] Implementar `POST /api/auth/login` en `AuthController.java` (depende de T027)
- [x] T029 [P] [US2] Crear `TokenStore.kt` (EncryptedSharedPreferences) en
      `android/app/src/main/java/com/walletapp/android/data/TokenStore.kt`
- [x] T030 [US2] Implementar `AuthApi.login` y `AuthRepository.login` (persiste el token vía
      `TokenStore`) (depende de T029)
- [x] T031 [P] [US2] Crear `LoginScreen.kt` en
      `android/app/src/main/java/com/walletapp/android/auth/ui/LoginScreen.kt`
- [x] T032 [US2] Extender `AuthViewModel` con el estado de login (depende de T030)
...
- [x] T033 [P] [US3] Test unitario de `LogoutUseCase` (inserta `jti` vía `RevokedTokenRepository`) en
      `auth/application/LogoutUseCaseTest.java`
- [x] T034 [P] [US3] Test de integración de `GET /api/auth/me` y `POST /api/auth/logout` (incluyendo
      reutilización de un token ya invalidado) en `AuthControllerIT.java`

### Implementation for User Story 3

- [x] T035 [US3] Implementar `GetCurrentUserUseCase` en `auth/application/GetCurrentUserUseCase.java`
- [x] T036 [US3] Implementar `GET /api/auth/me` en `AuthController.java` (usa la identidad resuelta
      por `JwtAuthFilter`, T016; depende de T035)
- [x] T037 [US3] Implementar `LogoutUseCase` en `auth/application/LogoutUseCase.java` (depende de T006,
      T007)
- [x] T038 [US3] Implementar `POST /api/auth/logout` en `AuthController.java` (depende de T037)
- [x] T039 [P] [US3] Añadir interceptor Retrofit que adjunta el token de `TokenStore` a cada request
      en `android/app/src/main/java/com/walletapp/android/di/NetworkModule.kt` (depende de T029)
- [x] T040 [US3] Implementar logout en `AuthRepository`/`AuthViewModel` (limpia `TokenStore`) (depende
      de T039)
...
- [x] T041 Ejecutar `./mvnw verify` y confirmar que `domain`/`application` de `auth` superan el 80% de
      cobertura (constitución, principio III) — completar tests donde falte antes de cerrar la feature
- [x] T042 [P] Documentar configuración de JWT (secreto, expiración) como variables de entorno en
      `backend/src/main/resources/application.properties` y en `README.md`
- [x] T043 Ejecutar [quickstart.md](quickstart.md) de punta a punta contra la implementación real
- [x] T044 [P] Documentar como mejora futura (no bloqueante) la purga periódica de filas expiradas en
      `revoked_tokens`
